...
function AssumptionsPanel() {
  return <section className="assumptions-panel panel" aria-labelledby="assumptions-title">
    ...
    <div className="assumptions-grid">
      ...
      <article><span className="assumption-index">04</span><h3>Bill posting</h3><p>Fixed bills post on the next business day when their scheduled date is not a business day. Window and flexible bills use their scheduled date in the baseline; eligible business-day dates are available for recommendations.</p></article>
...
function App() {
  ...
  const [form, setForm] = useState({ name: '', amount: '', date: relativeDate(0), balance: '', recurrence: 'MONTHLY' as RecurrenceKind, flexibility: 'FLEXIBLE' as 'FIXED' | 'WINDOW' | 'FLEXIBLE', windowStart: '15', windowEnd: '22', enabled: true })
  const [loading, setLoading] = useState(false)
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setShowForm(false)
      setEditingItem(null)
      setForm({ name: '', amount: '', date: relativeDate(0), balance: '', recurrence: 'MONTHLY', flexibility: 'FLEXIBLE', windowStart: '15', windowEnd: '22', enabled: true })
