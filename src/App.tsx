...
type View = 'Projection' | 'Accounts' | 'Income' | 'Bills' | 'Assumptions'
...
function App() {
  ...
  return <main className="app-shell">
    <header className="topbar">
      <div className="brand"><span className="brand-mark">cf</span><span>cashflow</span></div>
      <nav aria-label="Main navigation">{(['Projection', 'Accounts', 'Income', 'Bills', 'Assumptions'] as View[]).map((item) => <button className={activeView === item ? 'nav-item active' : 'nav-item'} key={item} onClick={() => setActiveView(item)}>{item}</button>)}</nav>
